...
class TestWebInterface:
    ...
    def test_run_method(
        self, mock_sidebar, mock_markdown, mock_set_page_config, web_interface
    ):
                        ...
                        with patch("tempfile.NamedTemporaryFile") as mock_temp:
                            with patch("builtins.open", mock_open_json(mock_result)):
                                with patch("pathlib.Path.unlink"):
                                    ...
                                    assert (
                                        mock_file.name
                                        in mock_session["processing_results"]
                                    )
    ...
    def test_display_raw_json(self, web_interface):
        ...
        mock_data = {"document_id": "test-doc", "filename": "test.pdf", "page_count": 5}
    ...
    @patch("streamlit.button")
    def test_batch_processing_page(
        self,
        mock_button,
        mock_checkbox,
        mock_slider,
    ...
    @patch("streamlit.button")
    def test_processing_history_page(
        self,
        mock_button,
        mock_metric,
        mock_columns,
