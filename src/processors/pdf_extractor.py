...
class PDFExtractor(BaseProcessor):
    ...
    def __init__(
        self, config: dict[str, Any] | None = None, ocr_threshold: int | None = None
    ):
        ...
        self.normalize_whitespace = self.config.pdf_extraction.text[
    ...
    def extract_pages(self, pdf_path: Path) -> list[PageContent]:
        ...
        if not isinstance(pdf_path, Path):
            raise ValidationError(
                "pdf_path must be a Path object", field_name="pdf_path"
            )
    ...
    def _apply_ocr_to_page(self, page: fitz.Page) -> str:
        ...
        try:
            ...
            if not text or len(text.strip()) < 5:
                raise OCRError("OCR produced insufficient text output", confidence=0.0)
            return text.strip()
    ...
    def _extract_with_pymupdf(self, pdf_path: Path) -> list[PageContent]:
        ...
        try:
            ...
            raise PDFProcessingError(
                f"PyMuPDF extraction failed: {e}", pdf_path=str(pdf_path)
            ) from e
        if not pages:
            raise PDFProcessingError(
                "No pages were successfully extracted from PDF", pdf_path=str(pdf_path)
            )
        return pages
    ...
    def _process_single_page(
        ...
        try:
            ...
            if len(text.strip()) < self.min_text_length:
                ...
                return None
            return PageContent(
                page_number=page_num + 1, raw_text=text, is_ocr_applied=needs_ocr
            )
        except Exception as e:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                for page_num, page in enumerate(pdf.pages):
                    try:
                        ...
                        if len(text.strip()) < self.min_text_length:
                            logger.debug(
                                f"Skipping page {page_num + 1} - insufficient text length"
                            )
                            continue
                        pages.append(
                            PageContent(
                        ...
                        logger.error(
                            f"Failed to process page {page_num + 1} with pdfplumber: {e}"
                        )
                        # Continue with next page instead of failing completely
                        continue
        except Exception as e:
            raise PDFProcessingError(
                f"pdfplumber extraction failed: {e}", pdf_path=str(pdf_path)
            ) from e
        if not pages:
            raise PDFProcessingError(
